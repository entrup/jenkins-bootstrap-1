...
folder("Terraform-Polly") {
    displayName("Terraform-Polly")
    description("Folder contining Terraform-Polly Jobs")
  }
freeStyleJob('Terraform-Polly/Terraform-Polly-Feature-Branch') {
  description("Job to run Terraform-Polly feature branch jobs")
  parameters {
    choiceParam {
        name("action")
    ...
    stringParam {
        name("branch")
        defaultValue("dev")
        description("Polly feature branch to run")
        trim(true)
    }
    stringParam {
  ...
  scm {
    git {
        remote {
            name ("terraform-polly")
            url('git@github.com:entrup/terraform-polly.git')
